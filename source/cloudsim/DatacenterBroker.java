...
public class DatacenterBroker extends CloudSim {
	...
	protected VirtualMachineList vmlist;
	...
	protected void removeVMs(int number) {
		...
		for(int i=0;i<this.vmlist.size();i++){
			if(this.vmMapping[i]>=0){
				if(vmsCreated==vmsDestroyed+1){
					System.out.print(GridSim.clock()+": "+this.get_name()+ ": Error, you cannot remove the last VM.\n");
					break;
				}
				else{
...
//Juan debug output				
	...
	protected void deleteVMsinDatacenter(int chosenDatacenter) {
		...
		int[]array=new int[3];
		array[0]=5;
		array[1]=this.vmlist.size()-1;
		array[2]=1;
		super.send(datacenterID[chosenDatacenter], GridSimTags.SCHEDULE_NOW, DatacenterTags.VM_DESTROY_ACK, array);
		this.vmsRequested=1;
		this.vmsAcks=0;
	}
