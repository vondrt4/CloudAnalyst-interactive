 ...
 */
public class CloudletSchedulerFIFO extends VMScheduler {
	...
	public double updateVMProcessing(double currentTime, double[]mipsShare) {
		...
		//Wondra may not work for multiple PEs per cloudlet
		double availableMI = getCapacity(mipsShare) * timeSpam;
		
		for (ResGridlet rcl : getCloudletExecList()) {
			//System.out.print("Cloudlet remaining and capacity before:"+rcl.getRemainingGridletLength()+" "+availableMI);
			if (availableMI>rcl.getRemainingGridletLength()){
				availableMI-=rcl.getRemainingGridletLength();
				rcl.updateGridletFinishedSoFar(rcl.getRemainingGridletLength());
				if (availableMI<0.01) {
					availableMI=0;
					break;
				}
				//System.out.println(" after: "+rcl.getRemainingGridletLength()+" "+availableMI);
			} else {
				rcl.updateGridletFinishedSoFar(availableMI);
				availableMI=0;
		...
		if (getCloudletExecList().size() == 0) {
			...
			//System.out.println("Active step size at: "+currentTime+" is: "+timeSpam+" queue: "+getCloudletExecList().size());
		}
		...
		//for (ResGridlet rcl : getCloudletExecList()) {
		if (getCloudletExecList().size()>0){
		    ResGridlet rcl = getCloudletExecList().get(0);
...
*/		
	...
	public double cloudletSubmit(Cloudlet cloudlet, double fileTransferTime) {
		...
		for (ResGridlet rcl1 : getCloudletExecList()) {			    
			estimatedFinishTime += rcl1.getRemainingGridletLength() / (getCapacity(currentMipsShare) * rcl1.getNumPE());
			}
